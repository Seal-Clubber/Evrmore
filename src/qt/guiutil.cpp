...
#include "evrmoreaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Evrmore address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new EvrmoreAddressEntryValidator(parent));
    widget->setCheckValidator(new EvrmoreAddressCheckValidator(parent));
}
...
bool parseEvrmoreURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no evrmore: URI
    if(!uri.isValid() || uri.scheme() != QString("evrmore"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!EvrmoreUnits::parse(EvrmoreUnits::EVR, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseEvrmoreURI(QString uri, SendCoinsRecipient *out)
{
    // Convert evrmore:// to evrmore:
    //
    //    Cannot handle this later, because evrmore:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("evrmore://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "evrmore:");
    }
    QUrl uriInstance(uri);
    return parseEvrmoreURI(uriInstance, out);
}

QString formatEvrmoreURI(const SendCoinsRecipient &info)
{
    QString ret = QString("evrmore:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(EvrmoreUnits::format(EvrmoreUnits::EVR, info.amount, false, EvrmoreUnits::separatorNever));
        paramCount++;
    }
...
bool openEvrmoreConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(EVRMORE_CONF_FILENAME);
    ...
    /* Open evrmore.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Evrmore.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Evrmore (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Evrmore (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Evrmore*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "evrmore.desktop";
    return GetAutostartDir() / strprintf("evrmore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a evrmore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Evrmore\n";
        else
            optionFile << strprintf("Name=Evrmore (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the Evrmore app
...
bool GetStartOnSystemStartup()
{
    CFURLRef evrmoreAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (evrmoreAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, evrmoreAppUrl);

    CFRelease(evrmoreAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Evrmore app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, evrmoreAppUrl, nullptr, nullptr);
    }
