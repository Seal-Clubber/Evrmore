 ...
 * between evrmored, and evrmore-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateEVRs(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), EVRMORE_CONF_FILENAME));
    if (mode == HMM_EVRMORED)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), EVRMORE_PID_FILENAME));
#endif
    ...
    if (showDebug)
    {
        ...
        strUsage += HelpMessageOpt("-enableminerdevfund", "Require the dev fund payment by miners in coinbase payments");
...
{
    // EVR-TODO: Update these URLs as needed
    const std::string URL_SOURCE_CODE = "<https://github.com/EvrmoreOrg/Evrmore>";
    const std::string URL_WEBSITE = "<https://evrmorecoin.org>";
...
/** Sanity checks
 *  Ensure that Evrmore is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Evrmore process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", EVRMORE_CONF_FILENAME)).string());
    ...
    GenerateEVRs(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
